...
Constructor injection is just ordinary function-call argument passing — there is no special engine support for "injecting" a dependency, it's the same argument-binding mechanism covered in the functions lesson, applied to a constructor instead of a free function; the "dependency injection" pattern is entirely a *design discipline* about who's responsible for constructing those arguments, not a distinct language feature. A minimal autowiring container works by using PHP's reflection API at runtime: `ReflectionClass::getConstructor()->getParameters()` inspects a class's constructor parameter list — including each parameter's declared type — without ever instantiating the class, and the container recursively resolves each typed parameter (by looking it up or recursively autowiring it) before finally calling `$reflectionClass->newInstanceArgs($resolvedArgs)`, all of which is genuinely runtime introspection of compiled class metadata, not compile-time code generation the way some statically-typed DI frameworks work. This is precisely why autowiring can't resolve scalar/primitive constructor parameters (an `int $timeout`) automatically — reflection can tell the container a parameter is typed `int`, but there's no object graph to recursively construct for a primitive, only a value to supply, which is why real DI containers require explicit configuration for those cases. The performance cost of reflection-based autowiring (walking the constructor signature via reflection on every resolution) is also why production-grade containers precompile the resolution plan once and cache it, rather than reflecting on every single object construction.
## 🔀 See this in another language

- [C# — 03 · Dependency Injection Deep Dive](https://sigilipelli.github.io/csharp-mastery-path/level-3/03-dependency-injection/)
