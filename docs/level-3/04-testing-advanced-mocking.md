...
PHPUnit's mock objects aren't your class with some methods swapped out — `createMock()` uses PHP's runtime class-generation capability (`eval()`-based or opcode-cache-friendly code generation, depending on version) to synthesize a brand-new class at test-run time that extends or implements your target, overriding every method with a generated body that records the call (arguments, call count) into an internal expectation-tracking object and returns whatever you configured via `willReturn()`. This is why you can mock an interface with no implementation at all — the generated class only needs the interface's method signatures to satisfy PHP's normal class-linking rules, and its method bodies do nothing but bookkeeping. Stubs and mocks differ in *when* PHPUnit checks their configured behavior: a stub's `willReturn()` value is just consulted every call with no expectation tracking, whereas a `expects($this->once())`-style mock configuration is verified only in the test's teardown phase, when PHPUnit walks through every mock's recorded call log and throws an `ExpectationFailedException` if any didn't match — a genuinely deferred assertion, not one checked at call time. This generated-class mechanism is also the root of PHP's specific mocking traps: `final` classes and methods can't have generated subclasses at all (a compile-time restriction the class-generation approach can't bypass), and static methods can't be intercepted this way either, since static calls resolve directly to the class entry's method table rather than through an object instance that a mock's `__call`-style dispatch could intercept.
## 🔀 See this in another language

- [PowerShell — 05 · Testing Advanced (Pester Mocking)](https://sigilipelli.github.io/powershell-mastery-path/level-3/05-testing-advanced-mocking/)
