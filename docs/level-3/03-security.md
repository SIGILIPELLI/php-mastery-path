...
`password_hash()` doesn't compute a hash directly the way `md5()` does — by default it runs the **bcrypt** algorithm (a deliberately slow, tunable key-derivation function built on the Blowfish cipher's key schedule), embedding a random salt and the chosen cost factor directly into the returned string, which is why the same password hashed twice produces two different-looking outputs that `password_verify()` can still both validate. `hash_equals()` exists because a naive `===` string comparison on hashes short-circuits at the first differing byte — an attacker measuring response-time differences over many requests can use that timing leak to guess a secret one byte at a time; `hash_equals()` is implemented to always compare every byte of both strings regardless of where they diverge, keeping execution time constant relative to input content rather than input similarity. `unserialize()` on untrusted input is dangerous because PHP's serialization format can encode arbitrary objects, including their exact class name — deserializing invokes that class's `__wakeup()` or `__destruct()` magic methods automatically as part of reconstructing the object, and if any class reachable in your autoloader has such a method with a dangerous side effect (deleting files, executing a command), an attacker who controls the serialized bytes can trigger it without ever calling that method directly — a genuine "object injection" vulnerability rooted in how deserialization is wired to the engine's magic-method dispatch. SQL injection defenses beyond prepared statements matter because things like table/column names and `ORDER BY` directions can't be parameterized in the wire protocol at all (only values can) — those must be validated against a fixed allow-list before ever reaching string interpolation, since no amount of "prepared statement" protects a query where the *structure*, not a value, is attacker-influenced.
## Exercise
