...
OPcache directly targets the exact compile-then-execute pipeline covered in the setup lesson: by default, PHP recompiles a script's source into a fresh opcode array on *every single request*, throwing that opcode array away the moment the request ends — a genuine, measurable amount of CPU work repeated identically thousands of times a day for a file that never changes. OPcache intercepts this by storing the compiled opcode array (plus resolved class entries and static data) in a **shared memory segment** that persists across requests and even across separate PHP-FPM worker processes, keyed by file path; on a cache hit, the engine skips lexing, parsing, and compiling entirely and jumps straight to executing the already-compiled opcodes, which is why enabling OPcache is typically the single biggest performance win available to a PHP application with zero code changes. It caches *compiled code*, never your application's data or output — a `remember()`-style cache (typically backed by APCu's in-memory key-value store, Redis, or a file) is a completely separate layer you build yourself, storing the *result* of an expensive computation (a DB query, an API call) keyed by some cache key, checked before doing the expensive work and populated after. Cache invalidation is hard precisely because OPcache and an application-level cache have no way to know your underlying data changed — OPcache only detects source-file changes via filesystem timestamp checks (often disabled in production for speed, requiring an explicit `opcache_reset()` on deploy), and a `remember()` cache has no idea the database row it cached has since been updated unless your code explicitly deletes or overwrites that key.
## Exercise
