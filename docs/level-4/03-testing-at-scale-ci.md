...
`phpunit.xml` replaces command-line flags because PHPUnit reads and parses this file once, at test-runner bootstrap, building the exact same configuration object internally that flags would populate — the file just makes the configuration inspectable and versionable instead of scattered across CI scripts. Grouping tests with `@group` (or the `#[Group]` attribute) works via the same reflection-based discovery mechanism as basic test discovery: PHPUnit reads each test method's attached group metadata during its discovery pass and filters the run set *before* executing anything, so a `--group fast` run genuinely never even instantiates the excluded test classes' `setUp()` — the filtering happens at discovery, not by running and then discarding results. Code coverage relies on a completely different mechanism than either of those: it needs Xdebug or PCOV, extensions that hook into the Zend Engine's opcode-execution loop itself, recording which specific lines' opcodes were actually executed during the run — this is why coverage reporting has real overhead (every executed opcode triggers a coverage-recording callback) and why it requires a specific extension rather than being a pure-PHPUnit feature; PHPUnit only aggregates and renders the raw line-hit data the extension collected. Running the suite in CI on every push works because a CI runner does nothing PHP-specific at all — it just executes `php vendor/bin/phpunit` as a fresh process in a fresh container, meaning your CI environment is subject to the exact same compile-per-invocation cost as any other PHP execution, just on someone else's machine, on a schedule triggered by your VCS webhook.
## Exercise
